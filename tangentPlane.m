...
if figurecheck == true
trimesh(cnt2,pts2(:,1),pts2(:,2),pts2(:,3),'FaceAlpha',1,'EdgeColor','k');
hold on
end
end

function result = optAngle(pts1,pts2,restrictface1,restrictface2)
%Inputs: agg1 - the stationary/fixed aggregate
%             agg2 - the aggregate that needs to be "attached"/shifted
a = linspace(0,pi/2);
optimalangle = [];
plane1 = tangentPlane(pts1,[],restrictface1,0,false);

parfor ang = 1:length(a)
    plane2 = tangentPlane(pts2,[],restrictface2,a(ang),false);
